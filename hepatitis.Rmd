...
###Apply pipeline from Harris Chapter 11.4

```{r}
hepatitis$Class <- as.factor(hepatitis$Class)
summary(hepatitis$Class)
```

Subset data for cases where Class = live. Then select the transformed component of the transcan object and subset (recall that the transcan object $transformed contains the matrix of transformed variables)

```{r}
subset <- hepatitis$Class %in% "live"
trans <- htrans$transformed[subset,]
hsub <- hepatitis[subset,] #subset rows where Class = live
```


Now fit binary logistic model on original variables 

```{r}
library(rms)
f <- lrm(Class ~ Age + Sex + Steroid + Antivirals + Fatigue + Malaise + Anorexia + Liver_big + Liver_firm + Spleen_palpable + Spiders + Ascites + Varices + Bilirubin + Alk_phosphate + Sgot + Albumin + Protime + Histology, data=hsub)
